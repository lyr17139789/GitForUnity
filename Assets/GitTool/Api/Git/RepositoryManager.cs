    ...
    public class RepositoryManager : IRepositoryManager
    {
        ...
        public ITask UpdateGitLog()
        {
            var task = GitClient.Log()
                .Then(logEntries => { GitLogUpdated?.Invoke(logEntries); });
            return HookupHandlers(task, false);
        }
        ...
        public ITask UpdateGitStatus()
        {
            var task = GitClient.Status()
                .Then(status => { GitStatusUpdated?.Invoke(status); });
            return HookupHandlers(task, false);
        }
        ...
        public ITask UpdateGitAheadBehindStatus()
        {
            ...
            var updateTask = new ActionTask<GitAheadBehindStatus>(taskManager, status => { GitAheadBehindStatusUpdated?.Invoke(status); });
        ...
        public ITask UpdateLocks()
        {
            var task = GitClient.ListLocks(false)
                .Then(locks => { GitLocksUpdated?.Invoke(locks); });
            return HookupHandlers(task, false);
        }
